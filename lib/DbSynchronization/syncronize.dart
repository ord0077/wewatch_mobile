...
class SyncronizationData {
  ...
  Future<List<ObservationModel>> fetchAllInfo()async{
    final dbClient = await conn.db;
    List<ObservationModel> contactList = [] ;
    try {
      final maps = await dbClient.query(SqfliteDatabaseHelper.observationTable );
      for (var item in maps) {
        contactList.add(ObservationModel.fromMap(item));
      }
    } catch (e) {
      print(e.toString());
    }
  ...
  Future saveToMysqlWith(List<ObservationModel> contactList) async {
...
//    _onLoading();
      ...
      try{
        if (response.statusCode==200) {
          ...
          await Controller().delete('observationTable',contactList[i].id);
