...
class Covid19Sync {
  ...
  Future<List<CovidModel>> fetchAllInfo()async{
    final dbClient = await conn.db;
    List<CovidModel> contactList = [] ;
    try {
      final maps = await dbClient.query(SqfliteDatabaseHelper.covid19Table );
      for (var item in maps) {
        contactList.add(CovidModel.fromMap(item));
      }
    } catch (e) {
      print(e.toString());
    }
  ...
  Future saveToMysqlWith(List<CovidModel> contactList) async {
    for (var i = 0; i < contactList.length; i++) {
//      String img = "jpg," + new String.fromCharCodes(contactList[i].image);
      File imageFile = new File(contactList[i].image);
...
//    String basename = basename(imageFile.path);
      ...
      Map<String, dynamic> data = {
        ...
        "temperature":contactList[i].temperature,
        "staff_name":contactList[i].staffName,
        "company":contactList[i].company,
        "remarks":contactList[i].remarks,
        "image":fi,
      };
